...
vec3 computeBumpedNormal() {
	if(!useNormalMap)
		return gsOutput.Normal;
	vec3 Normal = normalize(gsOutput.Normal);
	vec3 Tangent = normalize(gsOutput.Tangent);
	Tangent = normalize(Tangent-dot(Tangent, Normal) * Normal);
	vec3 Bitangent = cross(Tangent, Normal);
	vec3 BumpMapNormal = material.bump_scaling * texture(NormalMap, gsOutput.TexCoord).xyz;
...
float computeShadowMap(int i, int shadowIndex) {
	...
	vec3 projCoords = gsOutput.LightSpacePosition[LightIndex[i]].xyz/gsOutput.LightSpacePosition[LightIndex[i]].w;
	...
	if(projCoords.z<=1.0) {
		...
		vec3 normal = normalize(gsOutput.Normal);
		vec3 lightDir = normalize(Lights[LightIndex[i]].position-gsOutput.Position);
...
float computeShadowCubeMap(int i, int shadowIndex) {
	...
	vec3 fragToLight = gsOutput.Position-Lights[LightIndex[i]].position;
	...
	if(Lights[LightIndex[i]].shadowType==HARD_SHADOW) {			// Hard point shadows
		...
		int samples = 20;
		float viewDistance = length(EyePosition-gsOutput.Position);
	...
	vec3  rayDir)  // camera to point vector
{
	...
	float viewDistance = distance(EyePosition, gsOutput.Position);
	float factor = 0.1f;
...
void main() {
	...
	// Texture coordinates.
	vec2 texCoords = (gsOutput.TexCoord.xy*material.tc_scale)+material.tc_offset;
	...
	if(useSkybox) {
		vec3 I = normalize(gsOutput.Position-EyePosition);

		// Reflection.
		vec3 Reflection_Vector = reflect(I, normalize(gsOutput.Normal));
		ReflectionColor = texture(Skybox, Reflection_Vector)*material.reflection;

		// Refraction.
		float ratio = 1.00/material.refraction;
		vec3 Refraction_Vector = refract(I, normalize(gsOutput.Normal), ratio);
	...
	// EyeToFrag
	vec3 viewVector = EyePosition-gsOutput.Position;
	...
	if(Shadeless) { // If shadeless avoid light computation.
		...
		for(int i = 0; i<NumLights; i++) {
			...
			if(Lights[i].isLocal) {
				lightDirection = normalize(Lights[i].position-gsOutput.Position);
				float lightDistance = distance(Lights[i].position, gsOutput.Position);
			...
			//vec3 viewDir = normalize(EyePosition-gsOutput.Position);
	...
	vec4 substateColor = mix(MinColor, MaxColor, gsOutput.SubstateColorRatio);	// Apply the min/max color ratio.
